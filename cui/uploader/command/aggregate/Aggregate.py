...
import web.log.Log
class Aggregate:
    def __init__(self, db, authData, repo):
        self.__db = db
        ...
        self.__repoDb = self.__db.Repositories[self.__authData.Username]
    def Show(self):
        self.__calc_date()
        web.log.Log.Log().Logger.info(("開始日: {0:%s}" % (self.__date_format)).format(self.__first_date))
        web.log.Log.Log().Logger.info(("最終日: {0:%s}" % (self.__date_format)).format(self.__last_date))
        web.log.Log.Log().Logger.info("期  間: {0} 日間".format(self.__date_span))
        self.__sum_repo_count = self.__repoDb['Repositories'].count()
        web.log.Log.Log().Logger.info("リポジトリ総数  : {0}".format(self.__sum_repo_count))
        web.log.Log.Log().Logger.info("リポジトリ平均数: {0} repo/日".format(self.__sum_repo_count / self.__date_span))
        self.__sum_code_size = self.__repoDb.query("select SUM(Size) SumSize from Languages;").next()['SumSize']
        web.log.Log.Log().Logger.info("コード平均量    : {0} Byte/日".format(self.__sum_code_size / self.__date_span))
        web.log.Log.Log().Logger.info("コード総量      : {0} Byte".format(self.__sum_code_size))
    ...
    def __show_sizes_by_languages(self):
        ...
        for lang in self.__repoDb.query('select Language, sum(Size) SumSize from Languages group by Language order by SumSize desc'):
            web.log.Log.Log().Logger.info(format_str.format(lang['Language'], lang['SumSize']))
