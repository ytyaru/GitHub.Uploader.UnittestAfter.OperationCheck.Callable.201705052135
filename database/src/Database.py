...
#    def __init__(self):
        ...
        self.__config.read(os.path.join(path_dir_root, 'config.ini'))
        # 相対パスなら
        if self.__config['Path']['DB'].startswith('./'):
            # python起動パスでなく、このプロジェクトをrootとする
            self.__path_dir_db = os.path.join(path_dir_root, self.__config['Path']['DB'][2:])
        else:
...
#        self.dir_db = os.path.abspath(config['Path']['DB'])
        ...
        if config['Path']['DB'].startswith('./'):
            ...
            self.dir_db = os.path.join(os.path.abspath(os.path.dirname(__file__)), config['Path']['DB'][2:])
        else:
            self.dir_db = os.path.abspath(config['Path']['DB'])
        """
