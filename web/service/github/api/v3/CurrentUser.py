...
class CurrentUser(object):
    ...
    def __GetRepoDb(self):
        try:
            return self.__db.Repositories[self.Name]
        finally:
            web.log.Log.Log().Logger.debug(self.__db.Repositories.keys())
    ...
    def Change(self, username):
        if None is not self.__db.Accounts['Accounts'].find_one(Username=username):
            self.__username = username        
    Name = property(__GetName, Change)
    
    def __GetPassword(self):
        return self.__db.Accounts['Accounts'].find_one(Username=self.Name)['Password']
    Password = property(__GetPassword)
    
    def __GetMailAddress(self):
        return self.__db.Accounts['Accounts'].find_one(Username=self.Name)['MailAddress']
...
#    Otp = property(__GetOtp)
    ...
    def __GetTwoFactorSecret(self):
        account_id = self.__db.Accounts['Accounts'].find_one(Username=self.Name)['Id']
#        print(account_id)
        two_fac = self.__db.Accounts['TwoFactors'].find_one(AccountId=account_id)
        if None is two_fac:
            return None
        else:
            return two_fac['Secret']
#        return self.__db.Accounts['TwoFactors'].find_one(AccountId=account_id)['Secret']
    TwoFactorSecret = property(__GetTwoFactorSecret)
    
    def GetAccessToken(self, scopes=None):
        sql = "SELECT * FROM AccessTokens WHERE AccountId == {0}".format(self.__db.Accounts['Accounts'].find_one(Username=self.Name)['Id'])
...
#        print(scopes)
#        print(sql)
        res = self.__db.Accounts.query(sql)
        ret = None
        for r in res:
#            print(r)
            ret = r
